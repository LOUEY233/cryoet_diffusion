...
def diffusion_defaults():
    defaults = dict(
        ...
        img_channels=1,  # Change img_channels to 1 for 3D grayscale volumes
        img_size=(32, 32, 32),  # Change img_size to 3D tuple
...
def get_diffusion_from_args(args):
    ...
    model = UNet(
        img_channels=args.img_channels,  # Use args.img_channels for 3D volumes
    ...
    diffusion = GaussianDiffusion(
        nn.DataParallel(model), args.img_size, args.img_channels, 10,  # Use args.img_size and args.img_channels for 3D volumes
        betas,
