...
class AttentionBlock(nn.Module):
    __doc__ = r"""Applies QKV self-attention with a residual connection for 3D data.
    
    Input:
        x: tensor of shape (N, in_channels, D, H, W)
    ...
    Output:
        tensor of shape (N, in_channels, D, H, W)
    Args:
    ...
    def forward(self, x):
        ...
        assert dot_products.shape == (b, d * h * w, d * h * w)
...
class ResidualBlock(nn.Module):
    __doc__ = r"""Applies two conv blocks with residual connection for 3D data. Adds time and class conditioning by adding bias after first convolution.
    ...
    Output:
        tensor of shape (N, out_channels, D, H, W)
    Args:
        in_channels (int): number of input channels
...
class UNet(nn.Module):
    __doc__ = """UNet3D model used to estimate noise.
